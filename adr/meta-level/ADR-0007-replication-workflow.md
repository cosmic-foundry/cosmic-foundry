...
- **Spec before code.** Before implementing a physics map (a
  solver, a reaction network, a tracer advector), document its
  `Map:` block — domain, codomain, operator, approximation
  parameters Θ, and convergence order p — in the implementing
  module. For Lane B/C maps (ADR-0013), a derivation document
  under `derivations/` is also required before code. Test
  problems from target codes (a KH setup, a 1D detonation) get
  specs under `replication/targets/<target>/problems/` that
  name the maps they require. Problem specs describe the
...
- **External grounding for physics capabilities.** A *physics
  ...
  2. *At least one test per physics capability must be externally
     ...
     grounding. A map whose only tests compare against
     engine-generated golden data is not externally grounded.

  3. *The external grounding source is recorded in the `Map:`
     block or accompanying derivation document.* Reviewers use
     it to confirm the expected answer independently before the
     test value is committed.
...
- **A living plan per replication target.** A checklist doc
  ...
  problems and names the maps each requires. Problem status
  fields (Proposed → Implementing → Verified → Drifted) update
  as work progresses; cross-target regressions surface through
  shared test infrastructure without separate bookkeeping.
