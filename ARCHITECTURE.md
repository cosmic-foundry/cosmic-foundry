...
`scripts/ci/check_numeric_imports.py`. Computation APIs may accept structural
protocols satisfied by symbolic objects, but they must phrase those protocols in
numeric terms and must not import the symbolic implementations.
...
1. Select the next computed uncovered atlas cell after finite-rate
   one-transition reaction-network dynamics promotion.
...
objects.  Atlas evidence is descriptor evidence: a descriptor is located in the
...
all-ones linear form is conserved.  `ReactionNetworkRHS` now exposes that
projection as a public structural constructor: computation does not import the
theory class, but it accepts any finite unit-transfer object with transitions
and a stoichiometry matrix.  The grounded projection evidence includes both a
...
That premise is now grounded by a four-species, one-transition irreversible
network whose closed-form finite-rate evolution is checked against explicit
time stepping.  `FiniteRateReactionNetworkDynamics` promotes only that narrow
finite-rate shape: conserved reaction-network descriptors with one reaction,
rank one stoichiometry, and zero equilibrium constraints.  Fully constrained
...
Newton root coverage is split by equality-constraint evidence: one region owns
