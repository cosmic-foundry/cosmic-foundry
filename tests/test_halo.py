...
def test_single_rank_fill_copies_1d_neighbor_ghosts() -> None:
    ...
    assert filled_left.segment(SegmentId(0)).payload[5] == pytest.approx(200.0)
    assert filled_left.segment(SegmentId(0)).payload[0] == pytest.approx(-10.0)
    assert filled_right.segment(SegmentId(1)).payload[0] == pytest.approx(103.0)
    assert filled_right.segment(SegmentId(1)).payload[5] == pytest.approx(-20.0)
...
def test_single_rank_fill_copies_2d_face_slab() -> None:
    ...
    assert jnp.allclose(
        filled.segment(SegmentId(0)).payload[3, 1:4],
...
def test_execute_returns_new_field_without_mutating_original() -> None:
    ...
    assert field.segment(SegmentId(0)).payload[5] == pytest.approx(-10.0)
    assert filled.segment(SegmentId(0)).payload[5] == pytest.approx(200.0)
...
def test_execute_rejects_required_extent_not_covered() -> None:
    ...
    field = DiscreteField("phi", (segment,), Placement({SegmentId(0): 0}))
