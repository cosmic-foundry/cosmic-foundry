...
def _calibrate_alpha(solver_class: type, fma_rate: float) -> float:
    ...
    Times Operator.assemble() + a.svd() + solver_class().solve() at _CALIB_N, takes the
    ...
    a_cal = Operator.assemble(disc(), mesh).matrix
    b_cal = Tensor([1.0] * n)
    solver = solver_class()
...
class _SolverClaim(CalibratedClaim[float]):
    ...
    def check(self, fma_rate: float) -> None:
        ...
        n = math.prod(self._mesh.shape)
        a = Operator.assemble(disc(), self._mesh).matrix
        b = Tensor([1.0] * n)
...
class _ConvergenceRateClaim(CalibratedClaim[float]):
    ...
    def check(self, fma_rate: float) -> None:
        ...
        for mesh in meshes:
            ...
            a_m = Operator.assemble(
                FVMDiscretization(mesh, self._flux, bc)(), mesh
            ).matrix
            _, s_vec, vt = a_m.svd()
