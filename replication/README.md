...
State which level this capability spec claims (see verification track
README for definitions):

- **Code verification** — MMS or analytical test; convergence order
  measured against a known answer from outside the codebase.
- **Solution verification** — Richardson extrapolation / GCI on a
  physically meaningful problem; discretization error quantified.
- **Model validation** — comparison to experimental or observational
  data; belongs in application repositories.
...
- **External reference:** (required for physics capabilities before
  ...
  A golden file generated by the engine does not qualify — that is a
  regression sentinel, not external grounding. See ADR-0007 §Amendments
